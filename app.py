...
# Configuración SQLALCHEMY con PyMySQL explícito
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:@localhost/db_empresa_2025'
db = SQLAlchemy(app)
...
####---------------------------------------------------------------------####
# Creamos las rutas para cada template
...
def nueva_lineaproducto():
    ...
    if request.method == "POST":
        id = request.form['id']
        productos_val = request.form['productos']
        forma_pago = request.form['forma_pago']
        coleccion = request.form['coleccion']
        nombre_sucursal_val = request.form['nombre_sucursal']
        sala_val = request.form['sala']
        horario = request.form['horario']

        try:
            new_sec = LineaProductos(id_producto=productos_val, sec_id=id, forma_pago=forma_pago,
                                     coleccion=coleccion, nombre_sucursal=nombre_sucursal_val,
                                     numero_sala=sala_val, id_seccionhe=horario)
            db.session.add(new_sec)
            db.session.commit()
            flash("Adición de línea de producto exitosa!", "success")
            return redirect('/lineaproducto')
        except SQLAlchemyError as e:
            db.session.rollback()  
            flash("Ocurrió un error mientras se añadía", "danger")
...
def nuevo_producto():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            crea_producto = Productos(id_producto=id_producto, descripcion=descripcion,
                                      nombre_categoria=nombre_categoria, precio=precio)
...
def nuevo_empleado():
    ...
    if request.method == "POST":
        ...
        nombre_categoria_val = request.form['nombre_categoria']
        ...
        try:
            ...
            crea_empleado = Empleados(id_empleado=id_empleado, nombre_empleado=nombre_empleado,
                                      nombre_categoria=nombre_categoria_val, salario=salario)
...
def nuevo_cliente():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            crea_cliente = Clientes(id_cliente=id, nombre_cliente=nombre_cliente,
                                    nombre_categoria=nombre_categoria_val, estado=0)
...
def otros():
    query1 = text("""
        SELECT clientes.id_cliente, clientes.nombre_cliente
        FROM clientes
        WHERE clientes.nombre_categoria = 'otros'
    """)
    query2 = text("""
        SELECT productos.id_producto, productos.descripcion, productos.precio
        FROM productos
        WHERE productos.nombre_categoria = 'otros'
    """)
    query3 = text("""
        SELECT empleados.id_empleado, empleados.nombre_empleado
        FROM empleados
        WHERE empleados.nombre_categoria = 'otros'
    """)
    ...
    try:
        ...
        producto = []
        empleado = []
    return render_template("otros.html", cliente=cliente, producto=producto, empleado=empleado)
